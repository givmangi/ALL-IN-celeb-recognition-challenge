...
LOCAL_TEST_FOLDER = "vggface2_train_split/val"  # your local split for testing

# which weights to use
WEIGHTS_PATH = "clip_finetuned_vgg.pt"      # or clip_finetuned.pt
...
# ── Embed a flat folder (all images directly inside, no subfolders) ───────────
...
# ── Corrected Submission + Log function ───────────────────────────────────────
def submit_and_log(res_dict, model_name, group_name="ALL-IN", url="", log_file="submission_log.txt"):
    """
    Submits results to the server using the correct 'images' payload format
    and appends execution logs to a local file.
    """
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{timestamp}] Connecting to server to submit '{group_name}'...")

    # CORRECTED KEY: Changed from 'results' to 'images' to match your baseline and server expectation
    payload = {
    ...
    try:
        # CORRECTED METHOD: Using json= instead of json.dumps string tricks sets headers correctly
...
if MODE == "competition":
    ...
    print("Computing Cosine Similarity via PyTorch...")
    # Convert numpy arrays back to tensors and move to your active device
    ...
    similarity_matrix = torch.matmul(q_tensor, g_tensor.T)

    # Get the top 10 highest similarity scores and their indices
    scores, top_indices = torch.topk(similarity_matrix, k=10, dim=1)

    # Move indices back to CPU for the dictionary mapping
    ...
    submit_and_log(
        res_dict=results,
        model_name="CLIP_ViT-L-14_clip_finetuned_vggDIFFSUBMISSION",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    print("Extracting query embeddings...")
    query_features = extract_embeddings_subfolders.__wrapped__ if False else None
